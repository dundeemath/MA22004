...
This interpretation makes clear that a confidence interval is not a probability statement about $\theta$ (as $\theta$ is not a rv). In Definition \@ref(def:confidence-interval-gen), note that the interval $C_n$ is random and the parameter $\theta$ is fixed.
