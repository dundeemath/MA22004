...
A $(1-\alpha)$ **confidence interval** for a parameter $\theta$ is a *random* interval $C_n = (a, b)$ where $a = a(X_1, \dots, X_n)$ and $b = b(X_1, \dots, X_n)$ are functions of the data such that 
\begin{equation}
...
A **large-sample confidence interval** at level approximately $100(1-\alpha)\%$ for the mean $\mu$ of any population when the variance is uknown is given by 
\begin{equation} 
...
```{definition, ci-norm-unknown-var}
