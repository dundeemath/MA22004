...
## Estimating proportions and rates {#sec-estimating-proportions-rates}
...
### Estimating rates {#sec-estimating-rates}
...
Since $\E[X] = \lambda t$ and $\Var[X] = \lambda t$ for a Poisson random variable, we obtain
$$
...
For sufficiently large expected counts (so that the Poisson distribution is not too skewed), the sampling distribution of $\widehat{\lambda}$ is approximately normal,
$$
...
To form an interval estimate, we replace the unknown $\lambda$ in the variance by the point estimate $\widehat{\lambda}$, giving the estimated standard error
$$
...
:::{#prp-ci-rate}
For a large observed count, an approximate $100(1-\alpha)\%$ confidence interval for the Poisson rate $\lambda$ is
$$
...
When the observed count $X$ is small, the normal approximation used in @eq-ci-rate can be unreliable. In that case, an exact confidence interval for the Poisson rate $\lambda$ can be constructed using $\chi^2$ critical values (via the exact sampling distribution of the Poisson count).
:::
...
:::{#prp-htest-rate}
Assume $X \sim \mathsf{Poisson}(\lambda t)$ and that $\lambda_0 t \ge 10.$  
...
:::{#exm-estimating-rate}
Suppose a hospital ward records $X=27$ asthma admissions over $t=18$ weeks. Estimate the weekly admission rate $\lambda$ and construct a $95\%$ confidence interval. Then test (at level $0.05$) whether the data provide evidence that the rate exceeds $\lambda_0=1$ admission per week.
...
Using @prp-ci-rate with $\alpha=0.05$ and $z_{0.025}=1.96$, the estimated standard error is
$$
\widehat{\se}(\widehat{\lambda}) = \sqrt{\frac{\widehat{\lambda}}{t}}
= \sqrt{\frac{1.5}{18}}
= 0.289,
$$
so the $95\%$ CI is
...
Now test $H_0:\lambda=1$ against $H_a:\lambda>1.$ Under $H_0$, the expected count over $t=18$ weeks is $\lambda_0 t = 18,$ and the $z$ statistic from @eq-htest-rate-z is
$$
