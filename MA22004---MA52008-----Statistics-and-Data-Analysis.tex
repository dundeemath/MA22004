...
\urlstyle{same}
\hypersetup{
  pdftitle={MA22004 / MA52008 --- Statistics and Data Analysis},
...
\title{MA22004 / MA52008 --- Statistics and Data Analysis}
\author{Dr Eric Hall}
\date{2026-01-15}
\begin{document}
\maketitle
...
Welcome to MA22004/MA52008 Statistics and Data Analysis at the
...
curated content for you to investigate.
...
\item
  Contact Hours: 2 lecture hours + 2 tutorial hours per week + 5 hours
  self-paced labs
\item
  Software: R (including RMarkdown/Quarto for reproducibile report
  writing)
\end{itemize}
...
This module aims to provide students with the skills and knowledge
required to analyse data and make statistical inferences using sample
data. Students will develop a solid foundation in:
...
Throughout the module, students will use R for data visualization,
statistical modeling, hypothesis testing, and creating reproducible
reports.

\section*{Indicative Content}\label{indicative-content}
\addcontentsline{toc}{section}{Indicative Content}

\markright{Indicative Content}

\subsection*{Data Analysis and
Exploration}\label{data-analysis-and-exploration}
\addcontentsline{toc}{subsection}{Data Analysis and Exploration}
...
\addcontentsline{toc}{subsection}{Sampling Distributions and Estimation}
...
\item
  ...
  \end{itemize}
\item
  One and two-sample inference (including paired data and
  bootstrapping)\\
\item
  ...
  \item
    Type I \& II errors, sensitivity, specificity, likelihood ratio,
    power of a test\\
  \end{itemize}
\item
...
\addcontentsline{toc}{subsection}{Linear Regression Models}
...
\subsection*{Computational Tools and
Reproducibility}\label{computational-tools-and-reproducibility}
\addcontentsline{toc}{subsection}{Computational Tools and
Reproducibility}
...
\tightlist
\item
  Explain how properties of populations relate to sample data and
  describe appropriate sampling techniques.\\
\item
  Use R to perform exploratory data analysis, generate summary
  statistics, and visualize data effectively.\\
\item
  Interpret correlation coefficients and apply Principal Component
  Analysis (PCA) for high-dimensional data analysis.\\
\item
  Construct and interpret confidence intervals for means, variances,
  proportions, and rates in one and two-sample situations.\\
\item
  Perform hypothesis tests, including goodness-of-fit tests, ANOVA, and
  regression model assessment.\\
\item
  Develop and assess simple and multiple linear regression models,
  including diagnostic testing.\\
\item
  Evaluate the reliability of statistical models and interpret the power
  of a test in an inference setting.\\
\item
  Engage in mathematical and statistical dialogue.\\
\item
  Use R to conduct analyses, test statistical models, and create
  reproducible research reports.
\end{itemize}

\section*{Lecture Plan}\label{lecture-plan}
\addcontentsline{toc}{section}{Lecture Plan}

\markright{Lecture Plan}
...
\bottomrule\noalign{}
\endlastfoot
1 & Orientation, EDA, Correlation, PCA, Reproducibility & Icebreaker, R
PCA \\
2 & Probability Distributions, Sampling Distributions, CLT & Candy
sampling \\
3 & Estimation, CI, Hypothesis Testing, Errors \& Power & Why 0.05? \\
4 & One-Sample Inference (CI \& Tests for Means, Proportions, Variances)
& How much water? \\
5 & Two-Sample Inference (Unpaired Cases) & UN \& Africa \\
6 & Two-Sample Inference (Paired vs.~Unpaired, Variance Testing) &
Tennis ball challenge \\
7 & One-Way ANOVA, Goodness-of-Fit Tests & R \texttt{aov} \\
8 & Simple \& Multiple Regression, LINE Assumptions & R \texttt{lm} \\
9 & Regression Inference, Model Diagnostics, Chi-square & R \texttt{lm},
Chi-square \\
10 & Quality Control, Bias Correction, Final Review & R 3-sigma control
charts \\
\end{longtable}

\section*{Assessment \& Coursework}\label{assessment-coursework}
\addcontentsline{toc}{section}{Assessment \& Coursework}

\markright{Assessment \& Coursework}

See MyDundee material.

\section*{Software and Resources}\label{software-and-resources}
\addcontentsline{toc}{section}{Software and Resources}

\markright{Software and Resources}
...
\item
  Lecture notes: \href{https://dundeemath.github.io/MA22004/}{Statistics
  and Data Analysis Lecture Notes}\\
\item
  Lab materials:
  \href{https://dundeemath.github.io/MA22004labs/}{Statistics and Data
  Analysis Labs}\\
\item
  Recommended Texts: see MyDundee Library resources.
\end{itemize}

\section*{Appendix: Mapping to CS1 Actuarial Statistics
Syllabus}\label{appendix-mapping-to-cs1-actuarial-statistics-syllabus}
\addcontentsline{toc}{section}{Appendix: Mapping to CS1 Actuarial
Statistics Syllabus}

\markright{Appendix: Mapping to CS1 Actuarial Statistics Syllabus}

This appendix provides a structured mapping of which CS1 topics are
addressed in which weeks of the module.
...
1.2.2 & Correlation measures, including Pearson's, Spearman's, and
Kendall's coefficients & 1 \\
1.2.3 & Principal Component Analysis (PCA) for dimensionality reduction
& 1 \\
2.6.1 & Sample statistics and their distributions & 2 \\
2.6.2 & Properties of sampling distributions (mean, variance, shape) &
2 \\
...
3.2.2 & Confidence intervals for means and variances (Normal and t),
proportions and rates (Binomial and Poisson), variances (Chi-square and
F) & 3, 4, 5, 6 \\
3.2.3 & Confidence intervals based on one and two-sample situations,
including paired data and bootstrapping & 3, 4, 5, 6 \\
3.3.1 & Hypothesis testing framework, including null/alternative
hypotheses, significance levels, errors, and power of a test & 3 \\
3.3.2 & Hypothesis tests for one and two-sample situations & 4, 5, 6 \\
3.3.3 & Goodness-of-fit tests, including one-way and two-way chi-square
tests & 7, 9 \\
4.1.1 & Simple and multiple linear regression models & 8 \\
4.1.2 & Assessing model adequacy, including residual analysis and
diagnostic tests & 9 \\
4.1.3 & Use of R to fit a linear regression model and interpret output &
8, 9 \\
\end{longtable}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-important-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-important-color-frame, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Important}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
Formatting should rely on the \emph{MA22004/MA52008 Lab Report}
template. This is available in the \texttt{MA22004labs} package, and
further instructions can be found in Lab 1.
...
lecture notes. These abbreviations are pretty standard and you might
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Variables and parameters}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Specifying a probability distribution}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Perspectives}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
distribution and facts about set theory.
...
probability of \(A\) and \(B\) occurring is equal to the probability
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{How do we link sample spaces and events to data?}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Observations vs rvs}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

\(X\) denotes a rv and \(x\) denotes a particular value of \(X\).

\end{tcolorbox}

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-important-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-important-color-frame, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{We measure probabilities of events}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\pandocbounded{\includegraphics[keepaspectratio]{00-prelim_files/figure-pdf/fig-eg-cdf-coin-flip-plot-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{00-prelim_files/figure-pdf/fig-eg-pmf-histogram-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-important-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-important-color-frame, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Events of probability zero}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-important-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-important-color-frame, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Equality in distribution versus equality of rvs}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-note-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-note-color-frame, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Its all Greek \ldots{} when it comes to kurtosis}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\chapter{Exploratory data analysis}\label{sec-eda}

Exploratory data analysis (EDA) is the process of understanding a data
set before performing formal inference or fitting a statistical model.
The goal is to identify the important features of the data (and any
problems) early.

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{In practice, EDA answers questions like:}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\section{A first look in R}\label{a-first-look-in-r}

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-note-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-note-color-frame, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Cyclamen Data}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

The \textbf{Cyclamen Data} contain 150 obs. of six features (sequence
{[}{]}, petal length {[}mm{]}, petal width {[}mm{]}, aperture diameter
{[}mm{]}, color {[}pink, white{]}, and group {[}alpha, beta, gamma,
delta, epsilon{]}) for \emph{cyclamen hederifolium}. These measurements
were taken by small groups of former MA22004 students using the same
measurement protocol at the University Botanic Gardens in the autumn of
2024.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cyclamen }\SpecialCharTok{|\textgreater{}} \FunctionTok{glimpse}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
A useful first step is to compute basic summaries and check that the
categorical variables have the expected levels.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cyclamen }\SpecialCharTok{|\textgreater{}} \FunctionTok{summary}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
The summary above includes the sample mean of the continuous variables
(e.g., \texttt{petal.width}) and the counts in each category for factors
(e.g., \texttt{group}).

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Missingness}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

In R, most functions will silently return \texttt{NA} if you have
missing values. When computing summaries, decide in advance how you want
to handle missingness (for example, removing incomplete rows for a
particular calculation, or imputing missing values).
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cyclamen }\SpecialCharTok{|\textgreater{}} \FunctionTok{summarise}\NormalTok{(}\FunctionTok{across}\NormalTok{(}\FunctionTok{everything}\NormalTok{(), }\SpecialCharTok{\textasciitilde{}} \FunctionTok{sum}\NormalTok{(}\FunctionTok{is.na}\NormalTok{(.))))}
\end{Highlighting}
\end{Shaded}
...
In EDA we typically combine numerical summaries with visual summaries.
The next few sections outline basic visualisations.

\section{Histograms and boxplots}\label{histograms-and-boxplots}

Below are some standard plots for petal measurements. Histograms help
you see shape (symmetry, skewness, multimodality), and boxplots
highlight spread and potential outliers.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cyclamen }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ petal.length)) }\SpecialCharTok{+}
    \FunctionTok{geom\_histogram}\NormalTok{(}\AttributeTok{bins =} \DecValTok{15}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{01-eda_files/figure-pdf/cyclamen-hist-l-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cyclamen }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ petal.width)) }\SpecialCharTok{+}
    \FunctionTok{geom\_histogram}\NormalTok{(}\AttributeTok{bins =} \DecValTok{15}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{01-eda_files/figure-pdf/cyclamen-hist-w-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cyclamen }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ aperture)) }\SpecialCharTok{+}
    \FunctionTok{geom\_histogram}\NormalTok{(}\AttributeTok{bins =} \DecValTok{8}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{01-eda_files/figure-pdf/cyclamen-hist-a-1.pdf}}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cyclamen }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{pivot\_longer}\NormalTok{(}\AttributeTok{cols =} \FunctionTok{c}\NormalTok{(petal.length, petal.width, aperture),}
...
\pandocbounded{\includegraphics[keepaspectratio]{01-eda_files/figure-pdf/cyclamen-boxplot-1.pdf}}

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-note-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-note-color-frame, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Outliers are not necessarily errors}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

A point that looks like an outlier is not automatically a data
\emph{error}. It may be a legitimate observation (for example, an
unusually large flower), but it is always worth checking whether the
measurement could have been recorded incorrectly (for example, were the
correct units used).
...
A common EDA question is whether distributions differ across a group
variable. Here, color is a natural grouping to start with.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cyclamen }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ colour, }\AttributeTok{y =}\NormalTok{ petal.length)) }\SpecialCharTok{+}
    \FunctionTok{geom\_boxplot}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{01-eda_files/figure-pdf/cylcamen-boxplot-colour-1.pdf}}

\section{Relationships between
variables}\label{relationships-between-variables}

For two numerical variables, a scatter plot is usually the first plot to
make.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cyclamen }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ petal.length, }\AttributeTok{y =}\NormalTok{ petal.width)) }\SpecialCharTok{+}
    \FunctionTok{geom\_point}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{01-eda_files/figure-pdf/cyclamen-scatter-1.pdf}}

Sometimes it is helpful to add a grouping variable (here:
\texttt{colour}) to see whether patterns differ between groups.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cyclamen }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ petal.length, }\AttributeTok{y =}\NormalTok{ petal.width, }\AttributeTok{shape =}\NormalTok{ colour)) }\SpecialCharTok{+}
    \FunctionTok{geom\_point}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{01-eda_files/figure-pdf/cyclamen-scatter-colour-1.pdf}}

\section{Correlation as a summary of
association}\label{correlation-as-a-summary-of-association}

Correlation coefficients give a one-number summary of association
between two numerical variables. Correlation does not imply causation!
...
\tightlist
\item
  \textbf{Pearson} correlation measures linear association (it is the
  default and is most directly connected to linear regression).
\item
  \textbf{Spearman} correlation measures monotone association by
  applying Pearson correlation to the ranks of the data.
\item
  \textbf{Kendall} correlation (Kendall's \(\tau\)) measures association
  using concordant and discordant pairs, and also measures monotone
  association.
\end{itemize}

In EDA, it is common to compute Pearson, Spearman, and Kendall
correlations together. If Pearson is small but Spearman/Kendall are
large (in magnitude), that often suggests a relationship that is
monotone but not well-approximated by a straight line.

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{A good EDA habit}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\item
  If the relationship looks monotone but not linear, or if outliers look
  influential, also compute Spearman and Kendall as a sensitivity check.
\end{enumerate}
...
\pandocbounded{\includegraphics[keepaspectratio]{01-eda_files/figure-pdf/fig-cyclamen-pairwise-scatter-1.pdf}}

}

\caption{\label{fig-cyclamen-pairwise-scatter}Pairwise scatterplot
matrix for cyclamen petal width, petal length, and aperture diameter
(using \texttt{GGally:ggpairs}). Points are shaped by flower colour. The
diagonal panels show each variable's distribution (one density per
colour); the upper panels summarise pairwise association using the
\emph{Pearson} correlation.}

\end{figure}%

The built-in function \texttt{cor} can be used to compute different
correlation coefficients by specifying the method. For the cyclamen
data, we will compute correlations between the three numerical flower
measurements (here we exclude \texttt{sequence}, which is an index
rather than a flower measurement).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cyclamen\_num }\OtherTok{\textless{}{-}}\NormalTok{ cyclamen }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{select}\NormalTok{(petal.width, petal.length, aperture)}
\end{Highlighting}
\end{Shaded}

\subsection{Pearson correlation
matrix}\label{pearson-correlation-matrix}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{cor}\NormalTok{(cyclamen\_num, }\AttributeTok{use =} \StringTok{"complete.obs"}\NormalTok{, }\AttributeTok{method =} \StringTok{"pearson"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
matrix}\label{spearman-correlation-matrix}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{cor}\NormalTok{(cyclamen\_num, }\AttributeTok{use =} \StringTok{"complete.obs"}\NormalTok{, }\AttributeTok{method =} \StringTok{"spearman"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
matrix}\label{kendall-correlation-matrix}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{cor}\NormalTok{(cyclamen\_num, }\AttributeTok{use =} \StringTok{"complete.obs"}\NormalTok{, }\AttributeTok{method =} \StringTok{"kendall"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
If Pearson is close to \(0\) but Spearman/Kendall are not, that often
suggests a relationship that is monotone but not well-approximated by a
straight line. If Pearson differs substantially from Spearman/Kendall,
it is also worth checking for outliers.

\section{Principal Component
Analysis}\label{principal-component-analysis}

Principal Component Analysis (PCA) is an exploratory technique for
understanding variation in multivariate numerical data. It constructs
new variables (principal components) which are linear combinations of
the original variables and which capture decreasing amounts of
variation.

For cyclamen, we will apply PCA to the three numerical flower
measurements: petal width, petal length, and aperture diameter.

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Rescaling}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

PCA is sensitive to the scale of the variables. When variables are
measured on different scales (or have very different spreads), it is
standard practice to \emph{standardise} them before doing PCA. In
\texttt{prcomp}, this is done with \texttt{scale.\ =\ TRUE}.

\end{tcolorbox}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cyclamen\_pca }\OtherTok{\textless{}{-}}\NormalTok{ cyclamen }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{select}\NormalTok{(petal.width, petal.length, aperture, colour)}

\NormalTok{pca\_fit }\OtherTok{\textless{}{-}}\NormalTok{ cyclamen\_pca }\SpecialCharTok{|\textgreater{}} 
    \FunctionTok{select}\NormalTok{(}\SpecialCharTok{!}\NormalTok{colour) }\SpecialCharTok{|\textgreater{}}
...
\subsection{Understanding the PCA
summary}\label{understanding-the-pca-summary}

For three variables, PCA produces three principal components (PC1, PC2,
PC3). The \texttt{summary()} output reports:

\begin{itemize}
\tightlist
\item
  \textbf{Standard deviation} of each principal component: this is the
  spread of the data along that component.
\item
  \textbf{Proportion of Variance} explained by each component: the
  fraction of the total variability captured by that component.
\item
  \textbf{Cumulative Proportion} or the fraction captured by the first
  \(k\) (reading left to right across the table).
\end{itemize}

In particular, the variance explained by PC\(k\) is
\texttt{pca\_fit\$sdev{[}k{]}\^{}2}, and the proportion of variance
explained is obtained by dividing by the total:
\[\mathrm{PVE}_k = \frac{\mathrm{sdev}_k^2}{\sum_{j=1}^3 \mathrm{sdev}_j^2} \,.\]
Because we used \texttt{scale.\ =\ TRUE}, each original variable is
standardised to have variance \(1\), so the total variance is
approximately \(3\) (up to rounding). This is why
\(\sum_{k=1}^3 \mathrm{sdev}_k^2 \approx 3\).
...
\tightlist
\item
  PC1 explains about \(62.2\%\) of the variation in the three
  measurements.
\item
  PC2 explains about \(19.4\%\), and PC3 about \(18.4\%\).
\item
  The first two components together explain about \(81.6\%\) of the
  variation.
\end{itemize}
...
\NormalTok{pca\_pve }\OtherTok{\textless{}{-}}\NormalTok{ pca\_var }\SpecialCharTok{/} \FunctionTok{sum}\NormalTok{(pca\_var)}

\NormalTok{pve\_tbl }\OtherTok{\textless{}{-}}
    \FunctionTok{tibble}\NormalTok{(}
        \AttributeTok{PC =} \FunctionTok{factor}\NormalTok{(}\FunctionTok{paste0}\NormalTok{(}\StringTok{"PC"}\NormalTok{, }\FunctionTok{seq\_along}\NormalTok{(pca\_pve)),}
        \AttributeTok{levels =} \FunctionTok{paste0}\NormalTok{(}\StringTok{"PC"}\NormalTok{, }\FunctionTok{seq\_along}\NormalTok{(pca\_pve))),}
        \AttributeTok{proportion =}\NormalTok{ pca\_pve,}
        \AttributeTok{cumulative =} \FunctionTok{cumsum}\NormalTok{(pca\_pve)}
\NormalTok{        )}
...
\pandocbounded{\includegraphics[keepaspectratio]{01-eda_files/figure-pdf/fig-cyclamen-scree-1.pdf}}

}

\caption{\label{fig-cyclamen-scree}\textbf{Scree plot}: proportion of
variance explained by each principal component in the cyclamen PCA.}

\end{figure}%

\subsection{PCA scores plot}\label{pca-scores-plot}

A useful EDA plot is the projection of the observations onto the first
two principal components. Here we colour by \texttt{colour} to check
whether colour is associated with the main modes of variation in the
measurements.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{scores }\OtherTok{\textless{}{-}}
    \FunctionTok{as\_tibble}\NormalTok{(pca\_fit}\SpecialCharTok{$}\NormalTok{x) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{bind\_cols}\NormalTok{(cyclamen\_pca }\SpecialCharTok{|\textgreater{}} \FunctionTok{select}\NormalTok{(colour))}

\NormalTok{scores }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{ggplot}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ PC1, }\AttributeTok{y =}\NormalTok{ PC2, }\AttributeTok{shape =}\NormalTok{ colour)) }\SpecialCharTok{+}
    \FunctionTok{geom\_point}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{01-eda_files/figure-pdf/pca-scores-1.pdf}}

The scatter plot above displays cyclamen PCA scores (PC1 vs PC2). Each
point is a flower; points are shaped by flower colour.

\subsection{Interpreting the principal
components}\label{interpreting-the-principal-components}

To interpret the components, we examine the \textbf{loadings} (the
coefficients defining each principal component as a linear combination
of the original variables). In \texttt{prcomp}, these are stored in
\texttt{pca\_fit\$rotation}.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{as\_tibble}\NormalTok{(pca\_fit}\SpecialCharTok{$}\NormalTok{rotation, }\AttributeTok{rownames =} \StringTok{"variable"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
A loading with larger magnitude means that the corresponding variable
contributes more strongly to that principal component.

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{PCA is not a hypothesis test}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

PCA is an exploratory tool to help you describe patterns and identify
dominant directions of variation. A common outcome is dimension
reduction: here, PC1 and PC2 capture about \(81.6\%\) of the variation,
so a two-dimensional view retains most of the information in the three
measurements.
...
a distribution depending on a parameter \(\theta\) whose value is
unknown. Let the rv \[T = g(X_1, \dots, X_n, \theta)\] be a function of
...
families of distributions will be indexed by one or more parameters and
include discrete distributions (Bernoulli, Binomial, Poisson, and
discrete Uniform) as well as continuous distributions (continuous
Uniform, Normal, Student's \(\mathsf{t}\), \(\chi^2\), and
\(\mathsf{F}\)).

\section{Bernoulli distribution}\label{sec-bernoulli-distribution}

The Bernoulli distribution describes a single trial with two possible
outcomes, often coded as \(1\) (success) and \(0\) (failure). It is the
basic building block behind proportions and binomial counts.

\begin{definition}[Bernoulli
distribution]\protect\hypertarget{def-bernoulli-dist}{}\label{def-bernoulli-dist}

A discrete rv \(X\) has a Bernoulli distribution with parameter
\(p \in (0,1)\) if \[
P(X=x; p) =
\begin{cases}
 p, & x=1,\\
...
}\end{equation} We write \(X \sim \mathsf{Bernoulli}(p)\).

\end{definition}

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Parameter}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{equation}\phantomsection\label{eq-bernoulli-mean-var}{
\mathbf{E}[X] = p
\qquad \text{and} \qquad
\mathop{\mathrm{Var}}(X) = p(1-p).
}\end{equation}

The pmf for two Bernoulli rvs with different values of \(p\) is shown in
Figure~\ref{fig-bernoulli-pmf}.

\begin{figure}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{02-sampling-distributions_files/figure-pdf/fig-bernoulli-pmf-1.pdf}}

}

\caption{\label{fig-bernoulli-pmf}The pmf of a Bernoulli rv for two
values of the success probability \(p\).}

\end{figure}%

\begin{example}[]\protect\hypertarget{exm-bernoulli}{}\label{exm-bernoulli}

Suppose that a component passes a quality check with probability
\(p = 0.95\). Let \(X=1\) if the component passes and \(X=0\) otherwise.
Then \(X \sim \mathsf{Bernoulli}(0.95)\), and the probability that the
component fails is \[
P(X=0) = 1 - p = 0.05.
\]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-important-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-important-color-frame, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Indicator variables}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

A Bernoulli rv is often used as an \textbf{indicator}: it takes value
\(1\) when a property holds, and \(0\) when it does not. This is exactly
the setup used for estimating proportions in
Section~\ref{sec-estimating-proportions}.

\end{tcolorbox}

\section{Binomial distribution}\label{sec-binomial-distribution}

The binomial distribution describes the number of successes in \(k\)
independent Bernoulli trials, each having the same success probability
\(p\).

\begin{definition}[Binomial
distribution]\protect\hypertarget{def-binomial-dist}{}\label{def-binomial-dist}

Let \(X\) denote the number of successes in \(k\) independent trials,
where each trial is a \(\mathsf{Bernoulli}(p)\) rv. Then \(X\) has a
binomial distribution with parameters \(k \in \mathbf{N}_{>}\) and
\(p \in (0,1)\) if
\begin{equation}\phantomsection\label{eq-binomial-pmf}{
P(X=x; k,p) = \binom{k}{x} p^x (1-p)^{k-x}, \quad x = 0,1,\dots,k.
}\end{equation} We write \(X \sim \mathsf{Binomial}(k,p)\).

\end{definition}

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Parameters}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

The binomial distribution has \textbf{two parameters}: the number of
trials \(k\) and the success probability \(p\).

\end{tcolorbox}

If \(X \sim \mathsf{Binomial}(k,p)\), then
\begin{equation}\phantomsection\label{eq-binomial-mean-var}{
\mathbf{E}[X] = kp
\qquad \text{and} \qquad
\mathop{\mathrm{Var}}(X) = kp(1-p).
}\end{equation}

The pmf for \(\mathsf{Binomial}(10,p)\) for several values of \(p\) is
shown in Figure~\ref{fig-binomial-pmf}.

\begin{figure}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{02-sampling-distributions_files/figure-pdf/fig-binomial-pmf-1.pdf}}

}

\caption{\label{fig-binomial-pmf}The pmf of
\(X \sim \mathsf{Binomial}(10,p)\) for several values of \(p\).}

\end{figure}%

\begin{example}[]\protect\hypertarget{exm-binomial}{}\label{exm-binomial}

A student answers \(k=10\) multiple-choice questions by guessing. Each
question has probability \(p=0.25\) of being correct. If \(X\) is the
number of correct answers, then \[
X \sim \mathsf{Binomial}(10, 0.25).
\] The probability of getting exactly \(3\) correct answers is \[
P(X=3) = \binom{10}{3}(0.25)^3(0.75)^7.
\] This can be computed in \texttt{R} as
\texttt{dbinom(3,\ size\ =\ 10,\ prob\ =\ 0.25)}.

\end{example}

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Binomial counts are sums of Bernoulli trials}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

If \(X_1,\dots,X_k \sim \mathsf{Bernoulli}(p)\) are independent, then \[
X = \sum_{i=1}^k X_i \sim \mathsf{Binomial}(k,p).
\] This is why binomial distributions appear naturally when counting the
number of times an event occurs in repeated trials.

\end{tcolorbox}

\section{Poisson distribution}\label{sec-poisson-distribution}

The Poisson distribution is widely used to model the number of events in
a fixed unit of exposure (often time or space), such as the number of
arrivals at a desk per hour or the number of faults detected per
kilometre.

\begin{definition}[Poisson
distribution]\protect\hypertarget{def-poisson-dist}{}\label{def-poisson-dist}

A discrete rv \(X\) has a Poisson distribution with parameter
\(\lambda > 0\) if
\begin{equation}\phantomsection\label{eq-poisson-pmf}{
P(X=x; \lambda) = \frac{e^{-\lambda}\lambda^x}{x!}, \quad x = 0,1,2,\dots
}\end{equation} We write \(X \sim \mathsf{Poisson}(\lambda)\).

\end{definition}

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Parameter}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

The parameter \(\lambda\) is the mean number of events in the unit of
exposure (e.g.~``per hour'').

\end{tcolorbox}

If \(X \sim \mathsf{Poisson}(\lambda)\), then
\begin{equation}\phantomsection\label{eq-poisson-mean-var}{
\mathbf{E}[X] = \lambda
\qquad \text{and} \qquad
\mathop{\mathrm{Var}}(X) = \lambda.
}\end{equation}

That is, the Poisson distribution has the distinctive property that the
\textbf{mean equals the variance}. The pmf for several values of
\(\lambda\) is shown in Figure~\ref{fig-poisson-pmf}.

\begin{figure}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{02-sampling-distributions_files/figure-pdf/fig-poisson-pmf-1.pdf}}

}

\caption{\label{fig-poisson-pmf}The pmf of a Poisson rv for several
values of the rate parameter \(\lambda\).}

\end{figure}%

\begin{example}[]\protect\hypertarget{exm-poisson}{}\label{exm-poisson}

Suppose the number of emails you receive in an hour is modelled as
\(X \sim \mathsf{Poisson}(\lambda)\) with \(\lambda = 2.5.\) The
probability of receiving no emails in the next hour is \[
P(X=0) = e^{-2.5}\frac{2.5^0}{0!} = e^{-2.5}.
\] In \texttt{R}, this is \texttt{dpois(0,\ lambda\ =\ 2.5)}.

\end{example}

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-note-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-note-color-frame, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{When does Poisson modelling make sense?}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

Poisson models are commonly used when (i) events occur one at a time,
(ii) they occur ``at random'' throughout the exposure period, and (iii)
the average event rate is roughly constant over the period being
observed.
...
The uniform distribution places equal weight on the items being sampled.
The items can be discrete or be a continuum.

\subsection{Discrete uniform
distribution}\label{sec-discrete-uniform-distribution}

The discrete uniform places equal probability on a \emph{finite} set of
possible values.

\begin{definition}[Discrete uniform
distribution]\protect\hypertarget{def-discrete-uniform}{}\label{def-discrete-uniform}

A discrete rv \(X\) has a discrete uniform distribution on the integers
\(\{a,a+1,\dots,b\}\) with \(a<b\), if
\begin{equation}\phantomsection\label{eq-discrete-uniform-pmf}{
P(X=x; a,b) =
\begin{cases}
\frac{1}{b-a+1}, & x \in \{a,a+1,\dots,b\},\\[4pt]
0, & \text{otherwise}.
\end{cases}
}\end{equation} We write \(X \sim \mathsf{Unif}\{a,\dots,b\}\).

\end{definition}

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Parameters}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

The parameters \(a\) and \(b\) determine the finite support
\(\{a,a+1,\dots,b\}\).

\end{tcolorbox}

If \(X \sim \mathsf{Unif}\{a,\dots,b\}\), then
\begin{equation}\phantomsection\label{eq-discrete-uniform-mean-var}{
\mathbf{E}[X] = \frac{a+b}{2}
\qquad \text{and} \qquad
\mathop{\mathrm{Var}}(X) = \frac{(b-a+1)^2 - 1}{12}.
}\end{equation}

A familiar example is the outcome of a fair six-sided die:
\(X \sim \mathsf{Unif}\{1,\dots,6\}\). The pmf is shown in
Figure~\ref{fig-discrete-unif-pmf}.

\begin{figure}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{02-sampling-distributions_files/figure-pdf/fig-discrete-unif-pmf-1.pdf}}

}

\caption{\label{fig-discrete-unif-pmf}The pmf of a discrete uniform rv
on \(\{1,2,3,4,5,6\}\) (a fair die).}

\end{figure}%

\begin{example}[]\protect\hypertarget{exm-discrete-uniform}{}\label{exm-discrete-uniform}

Let \(X\) be the outcome of a fair die roll. Then
\(X \sim \mathsf{Unif}\{1,\dots,6\}.\) The probability of rolling a
value at least \(5\) is \[
...
distribution}\label{sec-continuous-uniform-distribution}

Compared with the discrete version, the continuous uniform distribution
places equal weight across an interval.
...
\pandocbounded{\includegraphics[keepaspectratio]{02-sampling-distributions_files/figure-pdf/fig-normals-diff-mean-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-sampling-distributions_files/figure-pdf/fig-normals-diff-sd-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-important-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-important-color-frame, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Useful facts about normal variates}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Variances add}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\pandocbounded{\includegraphics[keepaspectratio]{02-sampling-distributions_files/figure-pdf/fig-example-norm-geq-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-sampling-distributions_files/figure-pdf/fig-example-norm-interval-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-important-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-important-color-frame, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Empirical rule (\(68-95-99.7\) rule)}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\pandocbounded{\includegraphics[keepaspectratio]{02-sampling-distributions_files/figure-pdf/fig-example-t-dist-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-important-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-important-color-frame, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Properties of \(\mathsf{t}\) distributions}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-note-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-note-color-frame, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Cauchy distribution}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\pandocbounded{\includegraphics[keepaspectratio]{02-sampling-distributions_files/figure-pdf/fig-example-chisq-dist-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Skew}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
variance (see Chapter~\ref{sec-anova}).
...
\pandocbounded{\includegraphics[keepaspectratio]{02-sampling-distributions_files/figure-pdf/fig-example-F-dist-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Where do the terms numerator and denominator df come from?}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\pandocbounded{\includegraphics[keepaspectratio]{03-basics-stat-infer_files/figure-pdf/fig-setosa-petal-lengths-1.pdf}}

}

\caption{\label{fig-setosa-petal-lengths}The distribution (counts) of
\(m = 50\) \emph{setosa} petal length measurements.}

\end{figure}%

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Loading datasets}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-note-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-note-color-frame, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note \ref*{nte-iris}: Iris Data}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Overloaded notation}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Best practice for reporting}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-note-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-note-color-frame, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note \ref*{nte-tree}: Cherry Tree Data}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\pandocbounded{\includegraphics[keepaspectratio]{03-basics-stat-infer_files/figure-pdf/fig-qq-plot-cherry-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{How do we tell whether a population is normal?}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Alternative notation}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\pandocbounded{\includegraphics[keepaspectratio]{03-basics-stat-infer_files/figure-pdf/fig-fifty-cis-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-important-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-important-color-frame, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{``Fail to reject'' versus ``accept''}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{What a \(P\)-value isn't\ldots{}}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{What if the sample size is close to the population size?}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

In Example~\ref{exm-htest-alpha}, \(X\) is a binomial random variable
because it can be modelled as \(m\) independent Bernoulli trials each
...
\pandocbounded{\includegraphics[keepaspectratio]{assets/images/churchill_ballot.jpg}}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-important-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-important-color-frame, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Elements of a statistical test}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\tightlist
\item
  \(P\)-value (based on \(T\)) is compared to the significance level
  \(\alpha.\)
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio]{04-infer-single-sample_files/figure-pdf/fig-ci-norm-known-var-code-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{You heard it here first\ldots{}}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Recall}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Rule of thumb}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\pandocbounded{\includegraphics[keepaspectratio]{04-infer-single-sample_files/figure-pdf/unnamed-chunk-1-1.pdf}}
...
The test yields a \(P\)-value of \(P = 0.037.\) This values is quite
small; if \(\mu \leq 3.5\), then the probability of obtaining the sample
mean value \(\widehat{\mu} = 3.758\) is only 0.037! Thus, the data
...
\pandocbounded{\includegraphics[keepaspectratio]{04-infer-single-sample_files/figure-pdf/fig-exm-htest-ttest-plot-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-important-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-important-color-frame, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Shapiro-Wilk normality test}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\pandocbounded{\includegraphics[keepaspectratio]{04-infer-single-sample_files/figure-pdf/fig-qq-plot-cherry-vol-1.pdf}}
...
\section{Estimating proportions and
rates}\label{sec-estimating-proportions-rates}

\subsection{Estimating proportions}\label{sec-estimating-proportions}
...
\subsection{Estimating rates}\label{sec-estimating-rates}

In many applications, the quantity of interest is not a proportion, but
a rate: the average number of events per unit of exposure (often time).
For example,
...
A standard model for event counts is the Poisson distribution. Suppose
that we observe events over an exposure period of length \(t\)
(e.g.~\(t\) days), and let \(X\) be the total number of observed events.
We model \[
X \sim \mathsf{Poisson}(\lambda t),
\] where \(\lambda > 0\) is the event rate per unit exposure.

A natural estimator for the rate is
\begin{equation}\phantomsection\label{eq-rate-estimator}{
\widehat{\lambda} = \frac{X}{t}\,.
}\end{equation}

Since \(\mathbf{E}[X] = \lambda t\) and
\(\mathop{\mathrm{Var}}[X] = \lambda t\) for a Poisson random variable,
we obtain \begin{equation}\phantomsection\label{eq-rate-mean-var}{
\mathbf{E}[\widehat{\lambda}] = \lambda
\qquad \text{and} \qquad
\mathop{\mathrm{Var}}[\widehat{\lambda}] = \frac{\lambda}{t}\,.
}\end{equation}

For sufficiently large expected counts (so that the Poisson distribution
is not too skewed), the sampling distribution of \(\widehat{\lambda}\)
is approximately normal, \[
\widehat{\lambda} \approx \mathsf{N}\!\left(\lambda,\; \frac{\lambda}{t}\right).
\]

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Rule of thumb}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

A common rule of thumb for normal approximations with Poisson counts is
that the expected count is ``large enough'', e.g. \[
\lambda t \gtrsim 10.
\] In practice, we often check this using the observed count \(X\),
i.e.~we look for \(X \geq 10\).

\end{tcolorbox}

To form an interval estimate, we replace the unknown \(\lambda\) in the
variance by the point estimate \(\widehat{\lambda}\), giving the
estimated standard error \[
\widehat{\mathsf{se}}(\widehat{\lambda})
= \sqrt{\frac{\widehat{\lambda}}{t}}\,.
\]

\begin{proposition}[]\protect\hypertarget{prp-ci-rate}{}\label{prp-ci-rate}

For a large observed count, an approximate \(100(1-\alpha)\%\)
confidence interval for the Poisson rate \(\lambda\) is
\begin{equation}\phantomsection\label{eq-ci-rate}{
\widehat{\lambda} \pm z_{\alpha/2}\sqrt{\frac{\widehat{\lambda}}{t}}\,.
}\end{equation}

\end{proposition}

\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-note-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-note-color-frame, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Exact intervals for small counts}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

When the observed count \(X\) is small, the normal approximation used in
Equation~\ref{eq-ci-rate} can be unreliable. In that case, an exact
confidence interval for the Poisson rate \(\lambda\) can be constructed
using \(\chi^2\) critical values (via the exact sampling distribution of
the Poisson count).

\end{tcolorbox}

We can also test hypotheses about a rate. Under the null hypothesis
\(H_0:\lambda=\lambda_0\), the expected count is \(\lambda_0 t\) and the
standard deviation of \(X\) is \(\sqrt{\lambda_0 t}\). Thus a natural
large-sample test statistic is
\begin{equation}\phantomsection\label{eq-htest-rate-z}{
Z = \frac{X - \lambda_0 t}{\sqrt{\lambda_0 t}} \approx \mathsf{N}(0,1)\,.
}\end{equation}

\begin{proposition}[]\protect\hypertarget{prp-htest-rate}{}\label{prp-htest-rate}

Assume \(X \sim \mathsf{Poisson}(\lambda t)\) and that
\(\lambda_0 t \ge 10.\)
...
If \(H_a : \lambda > \lambda_0\), then \(P\)-value is the area under
\(\mathsf{N}(0,1)\) to the right of \(z.\)

If \(H_a : \lambda < \lambda_0\), then \(P\)-value is the area under
\(\mathsf{N}(0,1)\) to the left of \(z.\)

If \(H_a : \lambda \neq \lambda_0\), then \(P\)-value is twice the area
under \(\mathsf{N}(0,1)\) to the right of \(|z|.\)

\end{proposition}

\begin{example}[]\protect\hypertarget{exm-estimating-rate}{}\label{exm-estimating-rate}

Suppose a hospital ward records \(X=27\) asthma admissions over \(t=18\)
weeks. Estimate the weekly admission rate \(\lambda\) and construct a
\(95\%\) confidence interval. Then test (at level \(0.05\)) whether the
data provide evidence that the rate exceeds \(\lambda_0=1\) admission
per week.

First, the point estimate is \[
\widehat{\lambda} = \frac{X}{t} = \frac{27}{18} = 1.5 \quad \text{admissions per week.}
\]

Using Proposition~\ref{prp-ci-rate} with \(\alpha=0.05\) and
\(z_{0.025}=1.96\), the estimated standard error is \[
\widehat{\mathsf{se}}(\widehat{\lambda}) = \sqrt{\frac{\widehat{\lambda}}{t}}
= \sqrt{\frac{1.5}{18}}
= 0.289,
\] so the \(95\%\) CI is \[
1.5 \pm 1.96(0.289)
= (0.93,\; 2.07).
\] We interpret this as: a plausible range for the true weekly admission
rate is between about \(0.93\) and \(2.07\) admissions per week.

Now test \(H_0:\lambda=1\) against \(H_a:\lambda>1.\) Under \(H_0\), the
expected count over \(t=18\) weeks is \(\lambda_0 t = 18,\) and the
\(z\) statistic from Equation~\ref{eq-htest-rate-z} is \[
z = \frac{27-18}{\sqrt{18}} = 2.12.
\] The \(P\)-value is the upper-tail area
\(P = 1-\Phi(2.12) \approx 0.017.\) Since \(P<0.05\), we reject \(H_0\)
at the \(0.05\) level. The data provide evidence that the admission rate
exceeds \(1\) admission per week.

\end{example}

The calculations above can be reproduced using the following \texttt{R}
code.
...
\NormalTok{lambda\_hat }\OtherTok{\textless{}{-}}\NormalTok{ x }\SpecialCharTok{/}\NormalTok{ t}
\NormalTok{se\_hat }\OtherTok{\textless{}{-}} \FunctionTok{sqrt}\NormalTok{(lambda\_hat }\SpecialCharTok{/}\NormalTok{ t)}

\NormalTok{alpha }\OtherTok{\textless{}{-}} \FloatTok{0.05}
\NormalTok{zcrit }\OtherTok{\textless{}{-}} \FunctionTok{qnorm}\NormalTok{(}\DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ alpha}\SpecialCharTok{/}\DecValTok{2}\NormalTok{)}

\NormalTok{ci }\OtherTok{\textless{}{-}}\NormalTok{ lambda\_hat }\SpecialCharTok{+} \FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{) }\SpecialCharTok{*}\NormalTok{ zcrit }\SpecialCharTok{*}\NormalTok{ se\_hat}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lambda0 }\OtherTok{\textless{}{-}} \DecValTok{1}
\NormalTok{z }\OtherTok{\textless{}{-}}\NormalTok{ (x }\SpecialCharTok{{-}}\NormalTok{ lambda0 }\SpecialCharTok{*}\NormalTok{ t) }\SpecialCharTok{/} \FunctionTok{sqrt}\NormalTok{(lambda0 }\SpecialCharTok{*}\NormalTok{ t)}
\NormalTok{p\_val }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{{-}} \FunctionTok{pnorm}\NormalTok{(z)}
...
\pandocbounded{\includegraphics[keepaspectratio]{04-infer-single-sample_files/figure-pdf/fig-exm-htest-chisq-plot-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{04-infer-single-sample_files/figure-pdf/unnamed-chunk-7-1.pdf}}
...
\chapter{Two-sample inferences}\label{sec-inference-two-samples}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Why a weighted average?}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Robustness}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]

Pooled \(t\) procedures are not robust if the assumption of equal
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Alternative lingo}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
the same, i.e., \(\mu_1 = \mu_2 = \mu_3,\) but there are a number of
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-note-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-note-color-frame, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note \ref*{nte-salary}: Average Salary Data}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\pandocbounded{\includegraphics[keepaspectratio]{06-anova_files/figure-pdf/fig-anova-samples-boxplots-1.pdf}}
...
Then for the function \(\overline{x}_2 - \overline{x}_1,\) \[
\begin{aligned}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Consider the following}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\pandocbounded{\includegraphics[keepaspectratio]{07-linear-models_files/figure-pdf/fig-linear-model-fit-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{07-linear-models_files/figure-pdf/fig-linear-model-residuals-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{07-linear-models_files/figure-pdf/fig-linear-model-sigma-large-v-small-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Why do we lose two degrees of freedom?}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\] is an unbiased estimator of \(\sigma^2\) and the random variable \[
...
\pandocbounded{\includegraphics[keepaspectratio]{07-linear-models_files/figure-pdf/fig-linear-model-correlation-1.pdf}}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Prediction versus confidence intervals}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
number of trials that result in category \(i\) is given by
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Notation}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-tip-color-frame, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Why divide by expected cell counts?}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-warning-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-warning-color-frame, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{What about partial information?}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\(n_{ij}\) and the expected cell count \(\widehat{e}_{ij}.\)
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-important-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-important-color-frame, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Default position}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-note-color!10!white, colback=white, coltitle=black, opacitybacktitle=0.6, rightrule=.15mm, bottomtitle=1mm, arc=.35mm, breakable, colframe=quarto-callout-note-color-frame, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note \ref*{nte-beer}: Beer Production Data}, titlerule=0mm, toprule=.15mm, left=2mm, bottomrule=.15mm, toptitle=1mm, leftrule=.75mm, opacityback=0]
...
\pandocbounded{\includegraphics[keepaspectratio]{09-quality-control_files/figure-pdf/fig-qc-beer-pH-norm-1.pdf}}
...
a_m = \frac{\sqrt{2} \Gamma(m/2)}{\sqrt{m-1}\Gamma\left((m-1)/2\right)}\,.
...
\pandocbounded{\includegraphics[keepaspectratio]{09-quality-control_files/figure-pdf/fig-qc-beer-control-chart-1.pdf}}
